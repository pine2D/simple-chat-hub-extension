...
同步脚本（check=检查漂移并给方向建议；pull=姊妹仓→本仓；push=本仓→姊妹仓；覆盖前展示差异，目标侧有未提交改动时拒绝执行）：
```bash
scripts/sync-adapters.sh {check|pull|push}
```
...
├── custom/hosts.js       chatHub 父页共享站点清单 __SCH_HOSTS（panel 广播与 status HUD 共用）
